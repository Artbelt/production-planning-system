<?php
// NP_view_cut.php — просмотр раскроя (read-only + печать) для plan_U3
...
$bales = [];
foreach ($rows as $r) {
    $baleId = (int)($r['bale_id'] ?? 0);
    if ($baleId <= 0) continue;
    if (!isset($bales[$baleId])) {
        $bales[$baleId] = [
            'bale_id' => $baleId,
            'material' => (string)($r['material'] ?? ''),
            'format' => (int)($r['format'] ?? 1200),
            'strips' => [],
            'total_width' => 0.0,
        ];
    }
...
// Группируем по материалам для вывода
$byMaterial = [];
foreach ($bales as $baleId => $bale) {
    $mat = trim((string)($bale['material'] ?? ''));
    if ($mat === '') $mat = 'Simple';
    $byMaterial[$mat][] = $baleId;
}
ksort($byMaterial, SORT_NATURAL | SORT_FLAG_CASE);
...
foreach ($bales as $bale) {
    $fmt = (int)($bale['format'] ?? 1200);
    if ($fmt === 1200) $countFormat1200++;
    elseif ($fmt === 199) $countFormat199++;
    elseif ($fmt === 1000) $countFormat1000++;
    elseif ($fmt === 222) $countFormat222++;

    $baleLength = !empty($bale['strips']) ? (float)($bale['strips'][0]['length'] ?? 0) : 0;
    if ($baleLength >= 999) $countLength1000++;
    elseif ($baleLength >= 599 && $baleLength < 999) $countLength600++;
    elseif ($baleLength >= 499 && $baleLength < 599) $countLength500++;
}
...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}
        ...
        @media (max-width: 768px){ .material-group{grid-template-columns:1fr} }
...
<div class="panel">
    ...
    <div class="stats">
        <span>Всего бухт: <b><?= (int)$totalBales ?></b></span>
        <span>Ширина 1200 мм: <b><?= (int)$countFormat1200 ?></b></span>
        <?php if ($countFormat199 > 0): ?><span>Ширина 199 мм: <b><?= (int)$countFormat199 ?></b></span><?php endif; ?>
        <?php if ($countFormat222 > 0): ?><span>Ширина 222 мм: <b><?= (int)$countFormat222 ?></b></span><?php endif; ?>
        <?php if ($countFormat1000 > 0): ?><span>Ширина 1000 мм: <b><?= (int)$countFormat1000 ?></b></span><?php endif; ?>
        <?php if ($countLength1000 > 0): ?><span>Длиной 1000 м: <b><?= (int)$countLength1000 ?></b></span><?php endif; ?>
        <?php if ($countLength600 > 0): ?><span>Длиной 600 м: <b><?= (int)$countLength600 ?></b></span><?php endif; ?>
        <?php if ($countLength500 > 0): ?><span>Длиной 500 м: <b><?= (int)$countLength500 ?></b></span><?php endif; ?>
    </div>
</div>
        ...
        <div class="balesList">
            <?php foreach ($byMaterial as $mat => $baleIds): ?>
                <div class="material-group">
                    <div class="material-header">Материал: <b><?= htmlspecialchars($mat) ?></b></div>
                    <?php foreach ($baleIds as $baleId):
                        $bale = $bales[$baleId];
                        $baleWidth = (float)($bale['format'] ?? 1200);
                        $leftover = max(0, round($baleWidth - (float)$bale['total_width'], 1));
                        $baleNum = array_search($baleId, array_keys($bales), true);
                        $baleNum = ($baleNum === false) ? $baleId : ($baleNum + 1);
                    ?>
                        <div class="card">
                            <div class="cardHead">
                                <div>
                                    <b>Бухта #<?= (int)$baleNum ?></b> ·
                                    Остаток: <b><?= fmt1($leftover) ?> мм</b> ·
                                    Формат: <b><?= (int)$bale['format'] ?> мм</b>
                                </div>
                            </div>
                            <table class="baleTbl">
                                <colgroup>
                                ...
                                <?php foreach ($bale['strips'] as $strip): ?>
                                    <tr>
                                        <td><?= htmlspecialchars((string)$strip['filter']) ?></td>
