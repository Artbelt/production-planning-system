...
try {
    ...
    if (isset($_POST['fact'])) {
        ...
        $stmt = $pdo->prepare("SELECT fact_count, history FROM corrugation_plans WHERE id = ?");
        $stmt->execute([$id]);
        ...
        if ($delta != 0) {
            ...
            $stmt = $pdo->prepare("UPDATE corrugation_plans SET fact_count = ?, history = ? WHERE id = ?");
            $stmt->execute([$fact, $historyJson, $id]);
        } else {
            // Если дельта 0, просто обновляем fact_count (на случай ручной корректировки)
            $stmt = $pdo->prepare("UPDATE corrugation_plans SET fact_count = ? WHERE id = ?");
    ...
    if (isset($_POST['status'])) {
        ...
        $stmt = $pdo->prepare("UPDATE corrugation_plans SET status = ? WHERE id = ?");
