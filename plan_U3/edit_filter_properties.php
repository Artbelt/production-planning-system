...
/* === ДОБАВИТЬ: если нажали "Загрузить", берём текущий filter_name как analog_filter === */
...
<form action="processing_add_round_filter_into_db.php" method="post" >
    <form action="processing_add_round_filter_into_db.php" method="post">
        <!-- если у тебя есть отдельный UPDATE-скрипт, просто поменяй action на него -->
        <input type="hidden" name="mode" value="update"> <!-- для бэкенда: это обновление -->

        <label><b>Наименование фильтра</b>
            <input type="text" name="filter_name" size="40" value="<?php echo $filter_name ?>">
            <!-- Кнопка загрузки существующих данных по этому же имени -->
            <button type="submit"
                    name="load_from_db"
                    formaction="<?php echo htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES); ?>"
                    formmethod="post">
                Загрузить
            </button>
        </label>
    ...
    <label><b>Вставка PP</b></label><p>
        ...
        <?php load_insertions() ?>
        <label style="background-color: red; color: white"><?php if (isset($analog_data['pp_insertion'])){
                echo $analog_data['pp_insertion'];
            } ?></label>

    <hr>
    <label><b>Групповая упаковка</b></label><p>
        <label> <input type="text" size="5" name="packing" value="<?php echo $analog_data['packing']?>"></label>
    <hr>
