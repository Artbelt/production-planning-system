...
$production_date_end   = $_POST['production_date_end'];
    ...
    foreach ($result as $variant){
        ...
        echo "<tr style=' border: 1px solid black'><td>".$variant['date_of_production'].'</td><td>'.$variant['name_of_filter'].'</td><td>'.$variant['count_of_filters'].'</td><td>'.$variant['name_of_order'].'</td><td>'.get_filter_data($variant['name_of_filter'])['packing'].'</td></tr>';
    }
