...
if (isset($_GET['action']) && $_GET['action'] === 'full_replanning') {
    ...
    try{
        ...
        $factOperations = [
            ...
            'corrugation_plans' => "SELECT DISTINCT filter_label as filter, SUM(fact_count) as total_fact FROM corrugation_plans WHERE order_number = ? AND fact_count > 0 GROUP BY filter_label",
        ...
        $clearFuture = [
            ...
            "DELETE FROM corrugation_plans WHERE order_number = ? AND plan_date >= ?",
        ...
        // 2.5. Сбрасываем статусы для заявки (без поля status для U3)
        ...
        $remainingWork = $pdo->prepare("
            SELECT filter, count as total_planned, 
                   ...
                   COALESCE((SELECT SUM(fact_count) FROM corrugation_plans WHERE order_number = o.order_number AND filter_label = o.filter), 0) as corr_fact,
...
if (isset($_GET['action']) && $_GET['action'] === 'clear') {
    ...
    try{
        ...
        foreach ([
                     ...
                     ['sql'=>"DELETE FROM corrugation_plans WHERE order_number=?", 'key'=>'corr'],
...
<script>
    ...
    async function clearOrder(order){
        ...
        try{
            ...
            alert('Готово. Удалено записей:\n' +
                ...
                'corrugation_plans: '+(data.aff?.corr ?? 0)+'\n' +
    ...
    function editCorrugationPlan(order){
        if (confirm(
            '⚠️ ВНИМАНИЕ!\n\n' +
            'Вы собираетесь изменить готовый план гофрирования.\n\n' +
