<?php
// NP_view_corrugation_plan.php — компактный просмотр плана гофрирования (read-only + печать) для plan_U3
...
// В plan_U3 фактическая таблица: corrugation_plans(order_number, filter, day_date, qty, ...)
$sql = "
SELECT
  day_date AS plan_date,
...
function sumDay($arr){ $s=0; foreach($arr as $x) $s+=(int)($x['count'] ?? 0); return $s; }
$total = 0; foreach ($rows as $r) $total += (int)($r['count'] ?? 0);
?>
<!DOCTYPE html>
<html lang="ru">
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}
        ...
        .grid{display:grid;grid-template-columns: repeat(auto-fill, minmax(260px, 1fr));gap:8px;align-items:start;}
                ...
                <div class="card">
                    ...
                    <table>
                        ...
                        <?php foreach ($items as $r): ?>
                            <tr>
                                <td class="left" title="<?= htmlspecialchars((string)$r['filter_label']) ?>"><?= htmlspecialchars((string)$r['base_filter']) ?></td>
                                <td><?= (int)($r['count'] ?? 0) ?></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
