<?php
// API для работы с задачами участка U3
// Все задачи хранятся в базе plan_u5, но фильтруются по department = 'U3'
...
// Подключаемся к базе plan_u5, где хранятся все задачи
...
// Для участка U3 всегда используем department = 'U3'
$department = 'U3';

// Получить список пользователей участка U3 для выпадающего списка
if ($action === 'get_users') {
    try {
        // Получаем пользователей участка U3 с ролью supervisor (мастер)
        $users = $db->select("
...
// Получить задачи для текущего пользователя участка U3
...
// Получить все задачи участка U3 (для директора)
...
if ($action === 'update_status' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Проверяем, что задача назначена текущему пользователю и относится к участку U3
        $stmt = $pdo->prepare("SELECT assigned_to, department FROM tasks WHERE id = ?");
        ...
        if ($task['department'] !== $department) {
            http_response_code(403);
            echo json_encode(['ok' => false, 'error' => 'Задача не относится к участку U3']);
            exit;
        }
...
if ($action === 'delete_task' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Проверяем права доступа
        $stmt = $pdo->prepare("SELECT assigned_by, department FROM tasks WHERE id = ?");
        $stmt->execute([$task_id]);
        $task = $stmt->fetch();
