...
<style>
    ...
    *{box-sizing:border-box}
    ...
    .panel{border:1px solid #ccc;border-radius:8px;padding:0;height:100%;overflow:hidden;background:#fff;display:flex;flex-direction:column}
    .panel h3{margin:0 0 6px;font:600 14px/1.2 Arial}
    .meta{margin:6px 0 8px}
    .meta span{display:inline-block;margin-right:10px}
    
    /* Sticky заголовок для панели с бухтами */
    #balesPanel .panelHead {
        position: sticky;
        top: 0;
        background: white;
        z-index: 20;
...
<div class="wrap">
    ...
    <div class="panel" id="balesPanel">
        <div class="panelHead">
            <div style="flex:1">
                <h3 style="margin:0 0 6px 0">Собранные бухты</h3>
                <div style="font-size:11px;color:#6b7280;display:flex;gap:12px;flex-wrap:wrap">
                    <span id="carbonStats" style="color:#374151">Угольный: <b>0</b> м (<b>0</b> бухт)</span>
                    <span id="whiteStats" style="color:#374151">Белый: <b>0</b> м (<b>0</b> бухт)</span>
                </div>
            </div>
...
<script>
    ...
    function renderBales(){
        const box=el('bales');
        if(!bales.length){box.textContent='Пока нет'; box.classList.add('quiet'); updateBalesStats(); return;}
        box.classList.remove('quiet');
    ...
    // Функция подсчета и обновления статистики по материалам
    function updateBalesStats() {
        let carbonCount = 0, whiteCount = 0;
        let carbonMeters = 0, whiteMeters = 0;
        
        console.log('updateBalesStats: total bales:', bales.length);
        
        bales.forEach((b, idx) => {
            const matUpper = (b.mat || '').toUpperCase().trim();
            const isCarbon = (matUpper === 'CARBON');
        ...
        // Округляем метры до целых
        carbonMeters = Math.round(carbonMeters);
        whiteMeters = Math.round(whiteMeters);
        
        // Вычисляем количество стандартных бухт (углянные по 300м, белые по 400м)
        const carbonBalesEquiv = (carbonMeters / 300).toFixed(2);
        const whiteBalesEquiv = (whiteMeters / 400).toFixed(2);
        
        console.log(`Итого: углянный=${carbonMeters}м (${carbonCount} раскроев = ${carbonBalesEquiv} бухт), белый=${whiteMeters}м (${whiteCount} раскроев = ${whiteBalesEquiv} бухт)`);
        
        // Обновляем отображение
        const carbonEl = document.getElementById('carbonStats');
        const whiteEl = document.getElementById('whiteStats');
        
        if (carbonEl) {
            carbonEl.innerHTML = `Угольный: <b>${carbonMeters}</b> м (<b>${carbonCount}</b> раскроев) <b>${carbonBalesEquiv}</b> бухты`;
        }
        if (whiteEl) {
            whiteEl.innerHTML = `Белый: <b>${whiteMeters}</b> м (<b>${whiteCount}</b> раскроев) <b>${whiteBalesEquiv}</b> бухты`;
        }
    }
