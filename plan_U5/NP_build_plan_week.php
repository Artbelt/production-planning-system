...
try{
    ...
    $stCorr = $pdoSug->prepare("
        SELECT 
            ...
            cp.count,
            cp.fact_count,
            COALESCE(sfs.build_complexity, 0) AS rate_per_shift,
    ...
    $corrugatedFilters = $stCorr->fetchAll();
    
    // Получаем уже распланированные позиции из build_plan
    $stAssigned = $pdoSug->prepare("
        SELECT source_date, filter, SUM(count) AS assigned
        FROM build_plan 
        WHERE order_number = ?
        GROUP BY source_date, filter
    ");
    $stAssigned->execute([$order]);
    $assignedMap = [];
    foreach($stAssigned->fetchAll() as $r){
        $assignedMap[$r['source_date'].'|'.$r['filter']] = (int)$r['assigned'];
    }
    
    // Группируем по датам
    $corrByDate = [];
    foreach($corrugatedFilters as $cf){
        $date = $cf['source_date'];
        if(!isset($corrByDate[$date])) $corrByDate[$date] = [];
        $corrByDate[$date][] = $cf;
    }
}catch(Throwable $e){
    $orderSuggestions = [];
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Проверка и автоматическое расширение если добавили в последний день
        function checkAndExpandIfNeeded(targetDay) {
            if (!allOrderMode) return; // Только в режиме "Вся заявка"
            
            const lastDay = fmtDate(addDays(weekStart, spanDays - 1));
            console.log('checkAndExpandIfNeeded: targetDay=', targetDay, 'lastDay=', lastDay);
            
            if (targetDay === lastDay) {
                spanDays += 1;
                console.log('Добавление в последний день - расширяем сетку. Новый spanDays:', spanDays);
                // Перерисовываем сетку с новым количеством дней
                setTimeout(() => renderWeek(false), 100);
            }
        }
        ...
        function calculateAllOrderDays(){
            ...
            if (allDates.size === 0) {
                // Если план пустой, показываем текущую неделю + 1 день
                spanDays = 8;
                console.log('calculateAllOrderDays: plan empty, using 8 days');
                return;
            }
            ...
            // Показываем все дни заявки + 1 пустой день для удобства планирования
            spanDays = Math.max(8, daysDiff + 1); // минимум 8 дней (неделя + 1), всегда +1 день после последнего
        ...
        function renderWeek(skipBusy){
            ...
            for(let i=0;i<spanDays;i++){
                ...
                ['1','2'].forEach(team=>{
                    ...
                    let topH = 0;
                    layout.forEach(r=>{
                        const realH = paintEvent(day, team, r, r._effH, topH);
                        topH += realH; // используем реальную высоту элемента, а не расчетную
                    });
                });
            }
        ...
        function paintEvent(day, team, row, effH, topH){
            ...
            // Возвращаем реальную высоту в часах для правильного накопления topH
            const realHeightH = heightPx / PX_PER_HOUR;
            ...
            // Возвращаем реальную высоту в часах для корректного накопления позиций
            return realHeightH;
        }
