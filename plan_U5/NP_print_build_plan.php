...
try {
    ...
    // Получаем список активных заявок (у которых есть план сборки и которые не скрыты)
    ...
    // утраивают позиции в просмотре плана.
    $stmt = $pdo->prepare("
        SELECT 
            bp.plan_date,
            bp.filter,
                        ...
                        <div class="items-container">
                            ...
                            <?php else: ?>
                                <?php foreach ($brigades[1] as $item): 
                                    ...
                                    $fact = $factByFilter[trim($item['filter'])] ?? null;
                                ?>
                                <div class="item" 
