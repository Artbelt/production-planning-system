...
if ($action === 'get_users') {
    try {
        ...
        // Получаем пользователей цеха с ролями supervisor и assembler
        $users = $db->select("
            SELECT DISTINCT u.id, u.full_name, u.phone, r.display_name as role_name
            FROM auth_users u
            JOIN auth_user_departments ud ON u.id = ud.user_id
            JOIN auth_roles r ON ud.role_id = r.id
            WHERE ud.department_code = ? 
            AND r.name IN ('supervisor', 'assembler')
