<?php
// NP_print_corrugation_plan_2.php — просмотр/печать плана гофрирования (U2) с опцией "факт"
...
// План: даты → позиции (агрегировано по фильтру)
$planByDate = [];
$datesSorted = [];
$factByBase = [];
$slotFills = [];
...
try {
    $stmt = $pdo->prepare("
        SELECT
            DATE(cp.plan_date) AS plan_date,
            cp.filter_label,
            COALESCE(cp.`count`, 0) AS qty,
            ppp.p_p_height AS height
        FROM corrugation_plan cp
        LEFT JOIN panel_filter_structure pfs
            ON TRIM(pfs.filter) = TRIM(SUBSTRING_INDEX(COALESCE(cp.filter_label,''), ' [', 1))
        LEFT JOIN paper_package_panel ppp
            ON ppp.p_p_name = pfs.paper_package
        WHERE cp.order_number = ?
        ORDER BY plan_date, cp.filter_label
    ");
    $stmt->execute([$order]);
    ...
    // Факт выполнения (если включен) — из manufactured_corrugated_packages
    if ($showFact) {
...
<head>
    ...
    <title>План гофрирования<?= $showFact ? ' (факт)' : '' ?> — <?= h($order) ?></title>
    <style>
    ...
    <?php else: ?>
        ...
        <div class="page-container">
            <div class="brigade-section" id="daysOnly">
                <div class="brigade-header">План гофрирования • Заявка: <?= h($order) ?><?= $showFact ? ' • Факт' : '' ?></div>
                            ...
                            <div class="items-container">
                                ...
                                <?php else: ?>
                                    ...
                                    ?>
                                        <div class="item"
                                             data-filter="<?= h($base) ?>"
                                             <?php if ($showFact && $fact): ?>title="Выполнено: <?= (int)$fact['manufactured'] ?> из <?= (int)$fact['planned'] ?>"<?php endif; ?>>
